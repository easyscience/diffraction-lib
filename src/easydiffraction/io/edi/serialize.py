...
_SCHEMA_VERSION_TAG = '_edi.schema_version'
_SCHEMA_VERSION = '1'
...
def section_to_edi(body: str) -> str:
    """
    ...
    marker = f'{_SCHEMA_VERSION_TAG} {_SCHEMA_VERSION}'
...
def edi_body_from_text(text: str) -> str:
    """
...
def _validate_schema_marker(block: object) -> None:
    """Validate the required Edi schema marker."""
    schema_version = read_cif_str(block, _SCHEMA_VERSION_TAG)
    if schema_version is None:
        msg = (
            f'Edi schema version marker {_SCHEMA_VERSION_TAG} is required. '
            'Saved project sections must be Edi files with a schema marker.'
        )
    ...
    try:
        ...
        msg = f'Edi schema version must start with an integer, got {schema_version!r}.'
        raise ValueError(msg) from exc

    if major_version != 1:
        msg = (
            f'Unsupported Edi schema version {schema_version!r}. '
            'Open this project with a compatible EasyDiffraction version '
            'and re-save it as Edi schema_version 1.'
        )
        raise ValueError(msg)
...
def _strip_schema_marker_lines(text: str) -> str:
    """Remove schema marker lines from Edi text."""
...
def _is_schema_marker_line(line: str) -> bool:
    ...
    tag = _SCHEMA_VERSION_TAG
    return stripped == tag or stripped.startswith((f'{tag} ', f'{tag}\t'))
