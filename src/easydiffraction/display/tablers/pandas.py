...
class PandasTableBackend(TableBackendBase):
    ...
    def _build_base_styles(self, color: str) -> list[dict]:
        ...
        Args:     color: CSS color value (e.g., ``#RRGGBB``) to use for
        borders and header accents.

        Returns:     A list of ``Styler.set_table_styles`` dictionaries.
        """
        return [
    ...
    def _build_header_alignment_styles(self, df, alignments) -> list[dict]:
        ...
        Args:     df: DataFrame whose columns are being rendered.
        alignments: Iterable of text alignment values (e.g., ``'left'``,
        ``'center'``) matching ``df`` columns.

        Returns:     A list of CSS rules for header cell alignment.
        """
        return [
            {
    ...
    def _apply_styling(self, df, alignments, color: str):
        ...
        Args:     df: DataFrame to style.     alignments: Iterable of
        text alignment values for columns.     color: CSS color value
        used for borders/header.

        Returns:     A configured pandas Styler ready for display.
        """
    ...
    def _update_display(self, styler, display_handle) -> None:
        ...
        Args:     styler: Configured DataFrame Styler to be rendered.
        display_handle: Optional IPython DisplayHandle used for in-place
        updates.
        """
    ...
    def render(
        ...
        'left').     df: DataFrame whose index is displayed as the first
        column.     display_handle: Optional IPython DisplayHandle to
        update an         existing output area in place when running in
        Jupyter.
        """
