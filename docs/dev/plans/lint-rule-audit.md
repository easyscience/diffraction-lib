...
- **Cross-reference:** the accepted
  ...
  thresholds as design guardrails (*refactor; do not raise thresholds or
  add `# noqa`*). The `PLR0913`/`PLR0914`/`PLR0915`/`PLR0917` items in
  ...
  by a per-file ignore, which is a *standing exception* to that ADR.
- **Recommendations are non-binding.** Every "keep disabled" entry in
  ...
  approval**, not a decision taken by this document. Where a *permanent*
  ...
  - **`PLR` complexity ignored for `tests/**`** — conflicts with
    `lint-complexity-thresholds.md`; making it permanent needs an ADR
    amendment (or a new ADR that scopes the test-file exception).
...
- **Baseline (current `develop` + baseline fix):** all three check tasks
  pass clean — `pixi run py-lint-check` → *All checks passed!*,
  `pixi run py-format-check` → *591 files already formatted*,
  `pixi run docstring-lint-check` (pydoclint) → no findings. So every
  violation below is attributable purely to a disabled rule.
- **Audit overlay (no file edits):** the disabled rules are enabled
  **at the command line** against the *unmodified, tracked*
  `pyproject.toml`, so the working tree is never touched and no
  `git checkout`-style restore is involved:
  - `--extend-select A,FIX,SLF,T20,TD,D100,D104,DTZ005` turns on the
    commented `select` families (`A`, `FIX`, `SLF`, `T20`, `TD`) plus
    the three *Temporary* global-ignore codes. (`--select` /
  ...
  - `--config "lint.per-file-ignores = { … }"` **replaces** the
    ...
    *Temporary* `tests/**` and `docs/**` entries while keeping the
...
`N805`, `PLE` are in the `tests/**` *Temporary* ignore block but flag
nothing. `ANN` is in the `docs/**` *Temporary* block but flags nothing
...
| `SLF001` | src | 517 | Large; mostly legitimate intra-package access between cooperating objects. Worth a *separate, focused* review later, not a blanket enable. |
...
- **Measurement method** is a non-destructive command-line overlay on
  the *unmodified* `pyproject.toml` (see [Methodology](#methodology)),
...
Recorded as a recommendation (not yet decided): the audit *supports*
...
1. **Scope of this PR:** keep it free of rule enablement (recommended)
   — baseline fix + this document + the regeneration helper only — or
   also land the Tier-A "clean wins" in the same PR?
...
5. **TODO policy:** is enabling the `TD004`/`TD005` *formatting* subset
...
- Future Tier-A PRs: `pyproject.toml` (`[tool.ruff.lint]`), 34 `src`
  ...
  and `ruff --fix`-driven edits across `tests/`.

## Implementation steps (Phase 1)

- [x] **P1.1 — Refresh tutorial baselines for `auto_estimate`.**
  Re-ran ed-2/ed-6/ed-20, regenerated their entries in
...
- [x] **P1.3 — Add `tools/lint_rule_audit.py` regeneration helper.**
  ...
  testability:** a *pure* `aggregate(records)` function (plus a
  `scope(filename)` helper and the family rollup) that takes parsed Ruff
  JSON records and returns the per-rule/scope/fixability totals, kept
  separate from the thin CLI shim that invokes Ruff via `subprocess` and
...
`tools/` script — covering the helper's *pure* aggregation logic with
...
**Additional, baseline-specific checks** — these *supplement* the
...
- `pixi run script-tests-checked` — a superset of `script-tests`: it
  ...
  Running it satisfies the mandatory `script-tests` step *and* confirms
...
| `SLF001` | private member accessed | src (517), tests (1313) | Keep in tests (the point of unit tests); src warrants a *separate* focused review |
