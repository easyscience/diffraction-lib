...
See [Implementation steps (Phase 1)](#implementation-steps-phase-1)
for the per-step checklist. High-level:
...
No new ADR is required. The ADR already mandates type-neutral
parameter names (`adp_iso`, `adp_11`…`adp_23`) interpreted through
...
Atomic displacement parameters today support four `AdpTypeEnum`
members: `Biso`, `Uiso`, `Bani`, `Uani`
...
(isotropic) and the always-present sibling collection
`atom_site_aniso` (`adp_11`…`adp_23`), with `atom_site.adp_type`
selecting B-vs-U and iso-vs-ani.

The **dimensionless β tensor** — the third standard anisotropic ADP
convention, used by FullProf, SHELX-era data, and cryspy internally —
is **not** accepted on input or emitted on output. Today:

- `AdpTypeEnum` has no `beta` member.
- The B↔U type-switch conversion in
  `AtomSite._convert_adp_values` is a pure scalar (`B = 8π²U`); it has
  no cell access and cannot do the β transform.
...
- The aniso `RangeValidator(ge=0.0, le=10.0)` forbids negatives — wrong
  for off-diagonal components of *any* convention, and β values are
...
- The crysfml backend (`_convert_structure_to_dict`) sends **only**
  ...
  therefore a pre-existing gap, not introduced here; β on crysfml is
  out of scope for this plan (see Open questions Q3).
...
with `B = 8π²U`. easydiffraction has **no reciprocal-cell helper**
today (the `cell` category stores only direct `a b c α β γ`), so one
must be added.
...
2. **β has no isotropic form.** `'beta'` always implies anisotropic.
   Switching *to* `beta` from an isotropic type seeds the tensor like
   the existing iso→ani path, then converts U→β; switching *from*
   `beta` to an isotropic type converts β→U and collapses the diagonal,
   reusing the existing collapse path.

3. **Cell-dependent conversion via a new easydiffraction geometry
   helper.** Add a reciprocal-length helper
   (`a* b* c*` from `a b c α β γ`) under
   `src/easydiffraction/utils/` (or `core/` if it must stay
   domain-free — it is pure geometry, so `core/` is acceptable). β↔U/B
   conversion routes through the parent structure's `cell`; when no
   parent cell is reachable (atom constructed in isolation) the type
   switch raises a clear error rather than silently producing wrong
   numbers — this is a boundary-input edge case per §Project Context.
   Do **not** depend on cryspy's reciprocal helper for the ed-side
   conversion.
...
6. **CIF tags.** Register `_atom_site_aniso.beta_11`…`beta_23` on the
   ...
   `adp_type == 'beta'` (check the full value, not the first letter),
   so the writer emits a `beta` loop and groups β atoms separately.
   On read, a CIF carrying `_atom_site_aniso.beta_*` sets
...
- [ ] **P1.1 — Extend the ADP ADR.** Add an *Extension* section to
  `type-neutral-adp-parameters.md` recording the β decision (decisions
  1–6 above): first-class `beta` type, cell-dependent conversion,
  dimensionless-units handling, validator relaxation, and the new CIF
  tags. Keep the original Decision/Consequences intact.
  Commit: `Extend type-neutral ADP ADR with beta tensor`
- [ ] **P1.2 — Reciprocal-cell helper.** Add a pure-geometry helper
  returning `(a*, b*, c*)` from `(a, b, c, α, β, γ)`. No domain imports.
  Commit: `Add reciprocal-cell length helper`
- [ ] **P1.3 — `AdpTypeEnum.BETA`.** Add the member and its
  `description()`. Update any exhaustive membership sets that must now
  include it (search `git grep -n "AdpTypeEnum\."`).
...
- [ ] **P1.4 — aniso category: validator, CIF names, display units.**
  ...
  `_atom_site_aniso.beta_*` to the `adp_ij` CIF handlers (decision 6);
  type-aware display units (decision 4).
  Commit: `Support beta tensor in atom_site_aniso category`
- [ ] **P1.5 — type-switch conversion.** Extend
  `_convert_adp_values` with β↔U/B branches using the reciprocal-cell
  helper and the parent `cell`; raise a clear error when no cell is
  reachable; wire iso↔β seeding/collapse.
  Commit: `Convert ADP values to and from the beta tensor`
- [ ] **P1.6 — cryspy passthrough.** Add the `BETA` branch in
  `_update_aniso_beta` (store β directly) and include `BETA` in
  `aniso_types`. Confirm Q2 before committing.
...
- [ ] **P1.7 — CIF writer family.** Extend `_adp_family()` to return
  ...
  family. Confirm round-trip read→write.
  Commit: `Emit beta tensor in the atom_site_aniso CIF loop`
...
Add/extend the tests listed under *Concrete files*, then run the gate.
...
- CIF round-trip: read a `_atom_site_aniso.beta_*` loop → `adp_type ==
  'beta'` → write emits the β loop unchanged.
...
Off-diagonal displacement components may now be negative, as the
physics requires.
