...
- P1.8 now keeps the calculator category and live backend in distinct
  private slots: `self._calculator_category` for the category and
  `self._calculator` for the live backend.
- P1.5 now separates the two-key peak alias context used for
  canonicalization from the wider supported-types filter context.
- P2.1a now points the obsolete rendering tests at the project-category
  test path and clarifies the calculation-to-calculator test move.

I found no remaining blocking or actionable issues in this static
re-review. The plan is accepted for implementation.
