...
- **Swallowed boundary error.** The `engine` setter (`base.py:56-76`)
  wraps the factory `create(new_engine)` in `try/except ValueError`, logs
  a warning, and `return`s — silently leaving the engine unchanged when a
  user passes an unsupported engine name. This is a user-input boundary
  that should fail loudly (raise), not warn-and-continue.
...
**Fix:** raise on an unsupported engine instead of warning-and-returning;
and decide whether renderer engine selection should follow the
switchable-category contract or be explicitly documented as exempt (a
display-only backend, not a domain switchable).
