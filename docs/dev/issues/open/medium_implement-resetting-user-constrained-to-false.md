...
the rest of the session even after the alias/constraint is deleted — so
a parameter the user expects to refine again is silently held fixed.
This is a persisted/live-state correctness gap, not just a missing
feature.
