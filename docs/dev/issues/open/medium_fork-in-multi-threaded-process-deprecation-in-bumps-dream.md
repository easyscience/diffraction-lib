...
- unit:
  `tests/unit/easydiffraction/analysis/minimizers/test_bumps_dream.py::test_run_solver_preserves_parameter_order_and_forwards_init`
- integration:
  `tests/integration/fitting/test_bumps_dream_support.py::test_run_solver_preserves_parameter_order_and_forwards_init`
  and the DREAM resume tests.

This is not just log noise: Python is hardening fork-while-threaded
(slated to change the default start method), and fork in a
multi-threaded parent genuinely risks child-process deadlocks — exactly
the `MPMapper` startup path EasyDiffraction drives.

**Tension with issue 95.** Issue 95 (Re-Enable DREAM Multiprocessing)
proposes _preferring_ a `fork` context for direct-script entry points.
...
**Depends on:** related to issue 95 (multiprocessing start-method
policy).
