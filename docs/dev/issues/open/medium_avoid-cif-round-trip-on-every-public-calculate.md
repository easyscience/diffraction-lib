...
parameter update) and the public path (`called_by_minimizer=False` ->
`_recreate_cryspy_obj`), which rebuilds the whole cryspy object from a
serialized CIF string (`str_to_globaln` + `get_dictionary`) on **every**
call. Profiling a single `project.analysis.calculate()` shows this
round-trip dominating, including cryspy's `get_dictionary` recomputing
the X-ray atomic form-factor table via slow Slater-orbital integrals
(`calc_transs`), which for neutron data is never used.
...
- `_recreate_cryspy_obj` / `_recreate_cryspy_dict` / `calculate_pattern`
  invalidation logic in `calculators/cryspy.py`.
