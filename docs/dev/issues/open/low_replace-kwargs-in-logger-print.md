...
use explicit keyword arguments"). `Logger.print` is a project-owned public
façade, so it should enumerate the Rich console options it actually
supports rather than passing an open `**kwargs`.
...
arguments it forwards, or document the framework-passthrough exception if
intentional.
