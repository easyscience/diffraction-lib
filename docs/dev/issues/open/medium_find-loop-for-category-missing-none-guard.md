...
directly, while the sibling helper `_has_cif_loop` defensively checks
for `None` and `hasattr(..., 'get_loop')` before calling. If gemmi's
`find_loop` returns a falsy/None-like reference for a tag that exists as
a non-loop scalar (a real hand-edited-CIF possibility — e.g. an
