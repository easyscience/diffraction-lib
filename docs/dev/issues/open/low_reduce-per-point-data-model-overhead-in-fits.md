...
holding guarded descriptors. The hot-path array properties
(`x`, `d_spacing`, `intensity_meas`, ...) rebuild numpy arrays from those
...
**Fix:** hold the invariant per-point inputs as backing numpy arrays
(x, measured intensity, sigma, d-spacing) and expose the per-point
objects as views, so array extraction is O(1) instead of an
object-by-object rebuild. This is a larger data-model change; capture it
here as the umbrella for the remaining per-point overhead.
...
**Recommended-priority note:** Marked **low** — biggest conceptual change
with diminishing returns after issues 172-174; worth doing only if fit
