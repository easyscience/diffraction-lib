...
`created` and `last_modified` parse the stored CIF string with
`strptime` using a fixed `'%d %b %Y %H:%M:%S'` format every time the
property is read. If a user hand-edits the project file and changes the
timestamp to any other format, reading these properties (or any
report/display path that touches them) raises a bare `ValueError` with
