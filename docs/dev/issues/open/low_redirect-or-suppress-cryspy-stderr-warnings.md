...
(`cryspy.py:195` and `:343`). The original TODO line references (`#L112`,
`#L184`) are stale — line 112 is now unrelated code.

**Remaining work:**

- Remove the two now-satisfied `# TODO: Redirect stderr to suppress
  Cryspy warnings.` comments that still sit directly above the working
  redirects (`cryspy.py:190`, `:338`).
