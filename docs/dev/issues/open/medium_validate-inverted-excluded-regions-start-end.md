...
`region_mask = (x >= start) & (x <= end)`, so a region with `start > end`
produces an all-False mask and is silently ignored. A scientist who
enters the bounds in the wrong order gets no exclusion and no feedback.
