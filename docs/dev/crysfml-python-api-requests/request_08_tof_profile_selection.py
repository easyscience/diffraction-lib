...
FULLPROF_JORGENSEN_VON_DREELE = fullprof_array([
    [12000, 4.63],
    [12100, 12.73],
    [12200, 190.66],
    [12300, 730.86],
...
FULLPROF_JORGENSEN = fullprof_array([
    [12000, 0.18],
    [12100, 0.23],
    [12200, 177.13],
    [12300, 760.12],
...
PHASE_Si  1
  Cell  5.432382  5.432382  5.432382  90.0  90.0  90.0
  SPGR  F d -3 m
  Atom  Si  Si  0.125  0.125  0.125  0.54095  1.0
  Contributes_to_patterns  1
  Scale_Factors  1.0
END_PHASE_Si
"""
...
def main() -> None:
    ...
    jvd = compare_to_fullprof(
        'current Jorgensen-von-Dreele profile name',
        SI_TOF_JVD_CFL,
        FULLPROF_JORGENSEN_VON_DREELE,
    )
    jorgensen = compare_to_fullprof(
        'requested selectable Jorgensen profile',
        SI_TOF_JORGENSEN_CFL,
        FULLPROF_JORGENSEN,
    )
    print_summary('Request 8 powder-CFL control', [control])
    print_summary('Request 8: TOF profile selection', [jvd, jorgensen])
    print('Note: this request is masked until request 7 makes TOF CFL intensities nonzero.')
