...
REQUESTED_CFL = CONTROL_CFL.replace(
    '  Contributes_to_patterns  1',
    (
        '  ! API unknown: CrysFML Fortran has single-crystal extinction '
        'code, but no CFL/Python powder-pattern input was found.\n'
        '  Contributes_to_patterns  1'
    ),
)
...
def main() -> None:
    ...
    requested = compare_to_fullprof(
        'with single-crystal extinction API unknown',
        REQUESTED_CFL,
        FULLPROF_CONTROL,
        x_shift=Y2O3_X_SHIFT,
