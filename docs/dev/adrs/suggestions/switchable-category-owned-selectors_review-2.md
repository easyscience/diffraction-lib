...
mixin. The ADR should either keep the mixin scoped to Family A and define
separate Family-B/C adapters, or define a small common "supported type
provider" protocol that each category implements before the mixin renders
the table.
...
context-local alias UX by adding category-specific alias hooks to
`type` assignment, CIF serialization, and `show_supported()`.
...
Sections ADR, which lists `ProjectConfig` children as `ProjectInfo` and
`Rendering`, with public `project.rendering` and saved `_rendering.*`
