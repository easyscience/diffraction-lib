...
class TestExtractProjectFromZip:
    ...
    def test_extracts_project_dir(self, tmp_path):
        """Returns path to the directory containing project.edi."""
        ...
        assert (tmp_path / 'out' / 'my_project' / 'project.edi').is_file()
    ...
    def test_extracts_to_temp_dir_by_default(self, tmp_path):
        ...
        assert 'project.edi' not in result  # returns parent dir, not file
    ...
    def test_raises_value_error_no_project_cif(self, tmp_path):
        """Raises ValueError when ZIP has no project.edi."""
        zip_path = tmp_path / 'bad.zip'
        with zipfile.ZipFile(zip_path, 'w') as zf:
            zf.writestr('data.dat', '1 2 3\n')

        with pytest.raises(ValueError, match=r'No project\.edi found'):
    ...
    def test_ignores_other_project_cif_in_destination(self, tmp_path):
        """Only finds project.edi from the zip, not pre-existing ones."""
        dest = tmp_path / 'data'
        # Pre-create another project directory in the destination
        other_project = dest / 'aaa_other' / 'project.edi'
