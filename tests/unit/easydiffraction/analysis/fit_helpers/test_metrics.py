...
def test_calculate_r_metrics_and_chi_square():
    ...
    standard_uncertainties = np.array([1.0, 2.0, 3.0])
    residuals = y_obs - y_calc

    r = M.calculate_r_factor(y_obs, y_calc)
    rb = M.calculate_rb_factor(y_obs, y_calc)
    rw = M.calculate_weighted_r_factor(y_obs, y_calc, standard_uncertainties)
...
def test_calculate_weighted_r_factor_uses_inverse_variance_weights():
    from easydiffraction.analysis.fit_helpers import metrics as M

    y_obs = np.array([10.0, 20.0])
    y_calc = np.array([9.0, 16.0])
    standard_uncertainties = np.array([1.0, 2.0])

    result = M.calculate_weighted_r_factor(
        y_obs,
        y_calc,
        standard_uncertainties,
    )

    # sqrt((1 * 1^2 + 1/4 * 4^2) / (1 * 10^2 + 1/4 * 20^2))
    assert np.isclose(result, np.sqrt(5.0 / 200.0))
