...
def test_how_to_access_parameters_prints_paths_and_uids(capsys, monkeypatch):
    ...
    a.display.parameter_edifa_tags()
    assert 'Edifa Tag' in (captured3.get('columns_headers') or [])
    edifa_rows = [' '.join(map(str, row)) for row in captured3.get('columns_data') or []]
    assert any('_catA.alpha' in r for r in edifa_rows)
