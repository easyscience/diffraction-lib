...
def test_module_import():
    import easydiffraction.analysis.minimizers.lmfit_least_squares as MUT

    assert MUT.__name__ == 'easydiffraction.analysis.minimizers.lmfit_least_squares'
...
def test_is_subclass_of_lmfit():
    ...
    assert issubclass(LmfitLeastSquaresMinimizer, LmfitMinimizer)


def test_default_method():
    from easydiffraction.analysis.minimizers.lmfit_least_squares import LmfitLeastSquaresMinimizer

    m = LmfitLeastSquaresMinimizer()
...
def test_prepare_and_sync(monkeypatch):
    ...
    class FakeResult:
        def __init__(self):
            self.params = {'p1': FakeParam(7.0, stderr=0.3)}
            self.success = True
    ...
    minim = LmfitLeastSquaresMinimizer()
    params = [P('p1', 1.0)]
    ...
    assert params[0].uncertainty == 0.3
