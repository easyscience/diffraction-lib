...
def test_range_validator_bounds(monkeypatch):
    from easydiffraction.core.validation import AttributeSpec, DataTypes, RangeValidator
    from easydiffraction.utils.logging import log

    log.configure(reaction=log.Reaction.WARN)
    spec = AttributeSpec(type_=DataTypes.NUMERIC, default=1.0, content_validator=RangeValidator(ge=0, le=2))
    # inside range
    expected = 1.5
...
def test_membership_and_regex_validators(monkeypatch):
    from easydiffraction.core.validation import AttributeSpec, MembershipValidator, RegexValidator
    from easydiffraction.utils.logging import log
