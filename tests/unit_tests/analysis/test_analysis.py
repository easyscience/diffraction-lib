...
def mock_project():
    ...
    project.sample_models.get_all_params.return_value = [
        MagicMock(datablock_id="block1", category_key="cat1", collection_entry_id="entry1", name="param1", value=1.0, units="unit1", free=True, min=0.0, max=2.0, uncertainty=0.1)
    ]
    project.experiments.get_all_params.return_value = [
        MagicMock(datablock_id="block2", category_key="cat2", collection_entry_id="entry2", name="param2", value=2.0, units="unit2", free=False, min=1.0, max=3.0, uncertainty=0.2)
    ]
