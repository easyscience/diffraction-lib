...
def test_create_objective_function(mock_minimizer):
    ...
    objective_function = mock_minimizer._create_objective_function(
        parameters, sample_models, experiments, calculator
    )
    ...
    with patch.object(mock_minimizer, "_objective_function", return_value=[1.0, 2.0, 3.0]) as mock_objective:
        ...
        mock_objective.assert_called_once_with(
            {"param1": 1.0}, parameters, sample_models, experiments, calculator
        )
